...
关闭或刷新侧边栏后，本次整理**会丢失**——这是主动的产品取舍（避免演变成笔记库），详见
...
- **预设 Prompt**：输出用途 / 知识密度 / 文风 / 输出结构（单选）+ 附加要求（多选），
  每个按钮只映射一段作者维护、带版本的 Prompt，**插件内不做任何 AI 加工**。
...
不读取账号密码或 Cookie，不上传服务器，不写入长期本地数据库。详见 [docs/PRIVACY.md](docs/PRIVACY.md)。
