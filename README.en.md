...
- **Conversation reading**: a main-world path that reads ChatGPT's internal React data (more accurate for code blocks) + a DOM fallback, distinguishing user / AI. **Opening the side panel on a ChatGPT conversation auto-reads it — no manual "read" click**, and reopening lands on the messages instantly. The side panel strips Markdown markers on display (`##`, `**`, `>`, …) for clean reading; the original text handed to the AI is preserved.
